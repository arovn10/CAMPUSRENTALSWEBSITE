...
**IMS v2 — LIVE on prod (Phases 1–5, merged via PR #4). Nav is ON by default; `NEXT_PUBLIC_IMS_V2=0` is the kill-switch:**
...
**Env vars for IMS v2:** `NEXT_PUBLIC_IMS_V2` (nav kill-switch — set to `0` to hide v2; ON by default) · `EMAIL_FROM` (verified Resend sender — until set, transactional emails fall back to `noreply@campusrentalsllc.com` and may not send if that domain isn't Resend-verified; failures are logged, non-fatal) · `CRON_SECRET` (quarterly statements auth — endpoint returns 503 until set) · `NEXT_PUBLIC_SITE_URL` (links in emails). `RESEND_API_KEY` already exists.
...
> **Go-live status:** IMS v2 is LIVE on prod (PR #4 merged, nav on by default). **Still owed (post-launch):** (1) spot-check a real distribution / capital-account figure + a sample PDF statement against prod data; (2) set `EMAIL_FROM` to a Resend-verified sender + `CRON_SECRET` + `NEXT_PUBLIC_SITE_URL` on Lightsail so emails/quarterly statements actually send; (3) the still-pending security items (credential rotation, git-history purge). `NEXT_PUBLIC_IMS_V2=0` instantly hides v2 if anything looks wrong.
