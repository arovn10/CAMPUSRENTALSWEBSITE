 ...
 * This script uses the existing database connection and ensures no data is deleted
 */
...
const MIGRATION_FILE = path.join(__dirname, 'phase2-termsheet-student-housing-migration.sql');
...
async function runMigration() {
  ...
  try {
    ...
    if (dealsColumns.rows.length > 0) {
      ...
      dealsColumns.rows.forEach(row => console.log(`   - ${row.column_name}`));
      console.log('\n   Migration uses IF NOT EXISTS, so it\'s safe to continue.\n');
    } else {
      console.log('✅ No existing student housing columns found\n');
    }

    // Ask for confirmation (in non-interactive mode, we'll proceed)
    ...
    // PostgreSQL doesn't support multiple statements in a single query easily
    // So we'll execute the entire file as one query (PostgreSQL supports this)
    ...
    const universities = await client.query('SELECT id, name, shortName, city, state FROM universities ORDER BY name');
    console.log('📊 Universities in database:');
    if (universities.rows.length > 0) {
      universities.rows.forEach(uni => {
        console.log(`   ✓ ${uni.name} (${uni.shortName}) - ${uni.city}, ${uni.state}`);
      });
    } else {
