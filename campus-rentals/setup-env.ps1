# Setup Environment Variables for Campus Rentals Investor Portal
# This script will help you create the .env.local file

Write-Host "🔧 Setting up environment variables for Campus Rentals Investor Portal..." -ForegroundColor Green

# Check if we're in the right directory
if (-not (Test-Path "package.json")) {
    Write-Host "❌ Error: Please run this script from the campus-rentals directory" -ForegroundColor Red
    exit 1
}

# Generate a secure JWT secret
$JWT_SECRET = -join ((33..126) | Get-Random -Count 64 | ForEach-Object {[char]$_})

# Generate a GitHub webhook secret
$GITHUB_WEBHOOK_SECRET = -join ((33..126) | Get-Random -Count 32 | ForEach-Object {[char]$_})

# Create the .env.local file
$envContent = @"
# Database Configuration
# Prisma Accelerate Database URL
DATABASE_URL="prisma+postgres://accelerate.prisma-data.net/?api_key=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..g1IIGCJPW1gj-HKerDj_qygHG040s8V0O3LjxqQundw"

# JWT Configuration
JWT_SECRET="$JWT_SECRET"

# Google Maps API
# Replace with your actual Google Maps API key
NEXT_PUBLIC_GOOGLE_MAPS_API_KEY="your-google-maps-api-key"

# AWS Configuration
NEXT_PUBLIC_AWS_ACCESS_KEY_ID="15108fde5089768d6f68eaa70320e1f3442387814fa5ad1a20ff5e90b9894ee8"
NEXT_PUBLIC_AWS_SECRET_ACCESS_KEY="your_secret_access_key"
NEXT_PUBLIC_AWS_REGION="us-east-2"
NEXT_PUBLIC_S3_BUCKET_NAME="abodebucket"

# GitHub Webhook (if using auto-deploy)
GITHUB_WEBHOOK_SECRET="$GITHUB_WEBHOOK_SECRET"

# Environment
NODE_ENV="production"

# Prisma Configuration
PRISMA_GENERATE_DATAPROXY=true

# Database Pool Configuration
DATABASE_POOL_SIZE=10
DATABASE_CONNECTION_LIMIT=20

# Cache Configuration
CACHE_TTL=3600
CACHE_MAX_SIZE=100

# Rate Limiting
RATE_LIMIT_WINDOW_MS=900000
RATE_LIMIT_MAX_REQUESTS=100
"@

# Write the content to .env.local
$envContent | Out-File -FilePath ".env.local" -Encoding UTF8

Write-Host "✅ Environment file created: .env.local" -ForegroundColor Green
Write-Host ""
Write-Host "⚠️  IMPORTANT: You need to update the following values in .env.local:" -ForegroundColor Yellow
Write-Host "   1. NEXT_PUBLIC_GOOGLE_MAPS_API_KEY - Your Google Maps API key" -ForegroundColor White
Write-Host "   2. NEXT_PUBLIC_AWS_SECRET_ACCESS_KEY - Your AWS secret access key" -ForegroundColor White
Write-Host ""
Write-Host "🔐 Generated secure JWT_SECRET and GITHUB_WEBHOOK_SECRET" -ForegroundColor Green
Write-Host "🗄️  Database URL configured with Prisma Accelerate" -ForegroundColor Green
Write-Host ""
Write-Host "📝 Next steps:" -ForegroundColor Cyan
Write-Host "   1. Edit .env.local with your Google Maps API key and AWS secret" -ForegroundColor White
Write-Host "   2. Run: npm install" -ForegroundColor White
Write-Host "   3. Run: npx prisma generate" -ForegroundColor White
Write-Host "   4. Run: npx prisma db push" -ForegroundColor White
Write-Host "   5. Run: npm run db:seed" -ForegroundColor White
Write-Host "   6. Run: npm run build" -ForegroundColor White
Write-Host "   7. Restart your application" -ForegroundColor White
Write-Host ""
Write-Host "🚀 Your investor portal will be ready to use!" -ForegroundColor Green 