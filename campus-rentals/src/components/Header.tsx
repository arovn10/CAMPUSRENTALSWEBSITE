        ...
        <div className="flex items-center justify-between py-3.5">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {propertiesDropdownOpen && (
                <div className="absolute left-0 top-full z-50 mt-3 w-60 overflow-hidden rounded-2xl bg-white p-1.5 shadow-lift ring-1 ring-ink-900/5 animate-scale-in">
                  ...
                  >
                    Tulane / Loyola
                    <span className="block text-xs font-normal text-ink-500">New Orleans, LA</span>
                  </button>
                  <button
                  ...
                  >
                    FAU
                    <span className="block text-xs font-normal text-ink-500">Boca Raton, FL</span>
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-4 md:hidden">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-11 w-11 items-center justify-center text-ink-500 transition-colors hover:text-accent"
              aria-label="Campus Rentals on Instagram"
            >
            ...
            <button
              className="flex h-11 w-11 flex-col items-center justify-center bg-transparent focus:outline-none"
    ...
    {/* Mobile menu lives OUTSIDE the header: the glass nav's backdrop-filter makes
      ...
      >
        <div className="flex flex-col p-6 pt-10">
          <Link href="/" className="flex min-h-[44px] items-center text-lg font-medium text-ink-800 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>Home</Link>
          <div className="flex flex-col py-2">
            <span className="flex min-h-[32px] items-center text-xs font-semibold uppercase tracking-[0.15em] text-ink-500">Properties</span>
            <Link href="/tulane-housing" className="flex min-h-[44px] items-center pl-1 text-base text-ink-700 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>Tulane / Loyola — New Orleans</Link>
            <Link href="/fau-housing" className="flex min-h-[44px] items-center pl-1 text-base text-ink-700 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>FAU — Boca Raton</Link>
          </div>
          <Link href="/plaza" className="flex min-h-[44px] items-center text-lg font-medium text-ink-800 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>The Plaza</Link>
          <Link href="/about" className="flex min-h-[44px] items-center text-lg font-medium text-ink-800 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>About</Link>
          <Link href="/contact" className="flex min-h-[44px] items-center text-lg font-medium text-ink-800 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>Contact</Link>
          <Link href="/investors/login" className="flex min-h-[44px] items-center text-lg font-medium text-ink-800 transition-colors hover:text-accent" onClick={() => setMobileMenuOpen(false)}>Investors</Link>
          <a
            href={TENANT_LOGIN_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex min-h-[48px] items-center justify-center gap-1.5 rounded-full bg-ink-900 px-5 py-3 text-sm font-semibold text-white"
            onClick={() => setMobileMenuOpen(false)}
          >
            Tenant Login
            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
