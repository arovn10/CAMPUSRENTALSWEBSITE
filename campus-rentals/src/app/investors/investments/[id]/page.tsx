...
export default function InvestmentDetailPage() {
  ...
  const [showUpdateEntityContactModal, setShowUpdateEntityContactModal] =
    useState(false);
  const [pendingEntityContact, setPendingEntityContact] = useState<{
    id: string;
    name: string;
  ...
  const [pendingEntitySelectionIndex, setPendingEntitySelectionIndex] =
  ...
  const [editingEntityInvestment, setEditingEntityInvestment] =
    useState<any>(null);
  const [propertyEntityInvestments, setPropertyEntityInvestments] = useState<
    any[]
  >([]);
  ...
  const [globalWaterfallStructures, setGlobalWaterfallStructures] = useState<
    any[]
  >([]);
  const [allDistributions, setAllDistributions] = useState<any[]>([]);
  const [showWaterfallModal, setShowWaterfallModal] = useState(false);
  const [showCreateWaterfallModal, setShowCreateWaterfallModal] =
    useState(false);
  const [showCreateGlobalWaterfallModal, setShowCreateGlobalWaterfallModal] =
    useState(false);
  ...
  const [showEditGlobalWaterfallModal, setShowEditGlobalWaterfallModal] =
    useState(false);
  const [showEditDistributionModal, setShowEditDistributionModal] =
    useState(false);
  const [editingDistribution, setEditingDistribution] = useState<any>(null);
  const [editingWaterfallStructure, setEditingWaterfallStructure] =
    useState<any>(null);
  ...
  const [openOwnerBreakdownId, setOpenOwnerBreakdownId] = useState<
    string | null
  >(null);
  const [breakdownData, setBreakdownData] = useState<any>(null);
  const [showAllWaterfallStructures, setShowAllWaterfallStructures] =
    useState(false);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const investmentData = investments.find(
          (inv: Investment) => inv.id === params.id,
        );
        console.log(
          "Found investment data:",
          investmentData ? "Yes" : "No",
          "for ID:",
          params.id,
        );
        ...
        if (investmentData) {
          ...
          setEditData({
            ...
            acquisitionPrice:
              investmentData.property.acquisitionPrice?.toString() || "0",
            constructionCost:
              investmentData.property.constructionCost?.toString() || "0",
            totalCost: investmentData.property.totalCost?.toString() || "0",
            acquisitionDate: investmentData.property.acquisitionDate
              ? new Date(investmentData.property.acquisitionDate)
                  .toISOString()
                  .split("T")[0]
              : "",
            constructionCompletionDate: investmentData.property
              .constructionCompletionDate
              ? new Date(investmentData.property.constructionCompletionDate)
                  .toISOString()
                  .split("T")[0]
              : "",
            stabilizationDate: investmentData.property.stabilizationDate
              ? new Date(investmentData.property.stabilizationDate)
                  .toISOString()
                  .split("T")[0]
              : "",
      ...
      const docsResponse = await fetch(
        `/api/investors/documents?entityType=INVESTMENT&entityId=${params.id}`,
        {
          headers: {
  ...
  const handleUploadDocument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to upload document: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
    ...
    try {
      const requestBody = {
        ...
        investmentDate: editData.investmentDate
          ? new Date(editData.investmentDate)
          : null,
        ...
        acquisitionDate: editData.acquisitionDate
          ? new Date(editData.acquisitionDate)
          : null,
        constructionCompletionDate: editData.constructionCompletionDate
          ? new Date(editData.constructionCompletionDate)
          : null,
        stabilizationDate: editData.stabilizationDate
          ? new Date(editData.stabilizationDate)
          : null,
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to update investment: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (investment?.property?.id && createdEntity.id) {
          ...
          alert(
            "Entity created successfully! You can now add it to this project from the entity list.",
          );
        }
      } else {
        const errorData = await response.json();
  ...
  const fetchPropertyEntityInvestments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const propertyEntities = entityInvestments.filter(
          (ei: any) => ei.propertyId === investment.property.id,
        );
  ...
  const handleCreateMultiInvestorInvestment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const totalOwnership = multiInvestorData.investors.reduce(
        (sum, investor) =>
          sum + parseFloat(investor.ownershipPercentage || "0"),
        0,
      );
      ...
      const totalEntityAmount = parseFloat(
        multiInvestorData.totalInvestmentAmount || "0",
      );
      const totalInvestorAmounts = multiInvestorData.investors.reduce(
        (sum, investor) => sum + parseFloat(investor.investmentAmount || "0"),
        0,
      );

      if (totalInvestorAmounts > totalEntityAmount) {
        alert(
          "Total investor amounts cannot exceed the entity investment amount",
        );
        return;
      }

      // Check for negative amounts
      const hasNegativeAmounts = multiInvestorData.investors.some(
        (investor) => parseFloat(investor.investmentAmount || "0") < 0,
      );
      ...
      const response = await fetch("/api/investors/entity-investments", {
        ...
        body: JSON.stringify({
          ...
          ownershipPercentage: parseFloat(
            multiInvestorData.entityOwnershipPercentage,
          ),
  ...
  const handleEditEntity = async (entityInvestment: any) => {
    ...
    const ownersFromApi =
      ...
      entityInvestment.entityInvestmentOwners.length > 0
        ? entityInvestment.entityInvestmentOwners
    ...
    const entityInvestmentWithBreakdown = {
      ...
      entity: {
        ...
        entityOwners:
          ownersFromApi.map((owner: any) => ({
            ...owner,
            breakdown: owner.breakdown
              ? Array.isArray(owner.breakdown)
                ? owner.breakdown
                : []
              : null,
            showBreakdown:
  ...
  const handleDeleteEntity = async (entityInvestmentId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this entity investment? This action cannot be undone.",
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/investors/entity-investments/${entityInvestmentId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
      ...
      if (response.ok) {
        ...
        const updatedEntityInvestments = await fetch(
          "/api/investors/entity-investments",
          {
            headers: {
              Authorization: `Bearer ${getAuthToken()}`,
            },
        ...
        const propertyEntities = updatedEntityInvestments.filter(
          (ei: any) => ei.propertyId === investment?.property?.id,
        );
        if (
          ...
          alert(
            "All entities for this property have been deleted. Redirecting to dashboard.",
          );
          router.push("/investors/dashboard");
        }
      } else {
  ...
  const addEntityInvestor = async () => {
    ...
    setEditingEntityInvestment({
      ...
      entity: {
        ...
        entityOwners: [
          ...(editingEntityInvestment.entity.entityOwners || []),
          newOwner,
        ],
      },
    });
  ...
  const removeEntityInvestor = (index: number) => {
    ...
    const updatedOwners = editingEntityInvestment.entity.entityOwners.filter(
  ...
  const updateEntityInvestor = (index: number, field: string, value: any) => {
    ...
    if (field === "user") {
      updatedOwners[index] = {
        ...updatedOwners[index],
        user: { ...updatedOwners[index].user, ...value },
      };
    } else {
  ...
  const handleUpdateEntityInvestment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const invalidOwners = editingEntityInvestment.entity.entityOwners.filter(
        (owner: any) => {
          // Must have either a userId, an investorEntityId, or complete user info
          const hasUserId = owner.userId && owner.userId.trim() !== "";
          const hasInvestorEntity =
            owner.investorEntityId &&
            String(owner.investorEntityId).trim() !== "";
          const hasCompleteUserInfo =
            owner.user &&
            owner.user.firstName &&
            owner.user.firstName.trim() !== "" &&
            owner.user.lastName &&
            owner.user.lastName.trim() !== "" &&
            owner.user.email &&
      ...
      if (invalidOwners.length > 0) {
        ...
        alert(
          "Please ensure all investors have either been selected from the list or have complete information filled in.",
        );
        return;
      }

      // Validate ownership percentages
      const totalOwnership = editingEntityInvestment.entity.entityOwners.reduce(
        (sum: number, owner: any) => {
      ...
      // Validate individual amounts
      const totalEntityAmount = parseFloat(
        editingEntityInvestment.investmentAmount || 0,
      );
      const totalInvestorAmounts =
        editingEntityInvestment.entity.entityOwners.reduce(
          (sum: number, owner: any) => {
      ...
      // Check for negative amounts
      const hasNegativeAmounts =
        editingEntityInvestment.entity.entityOwners.some((owner: any) => {
      ...
      const requestBody = {
        ...
        ownershipPercentage: parseFloat(
          editingEntityInvestment.ownershipPercentage,
        ),
        entityOwners: editingEntityInvestment.entity.entityOwners.map(
          (owner: any) => ({
            ...owner,
      ...
      try {
        const ownersSummary = requestBody.entityOwners.map((o: any) => ({
          ...
          breakdownTotal: Array.isArray(o.breakdown)
            ? o.breakdown.reduce(
        ...
        console.log(
          "[Save] Raw owners payload (truncated):",
          JSON.stringify(requestBody.entityOwners).slice(0, 2000),
        );
      } catch (logErr) {
      ...
      const response = await fetch(
        `/api/investors/entity-investments/${editingEntityInvestment.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to update entity investment: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
  ...
  const handleNOICalculation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        "Other income in request:",
        requestBody.otherIncome,
        "Type:",
        typeof requestBody.otherIncome,
      );
      ...
      console.log(
        "NOI Calculation using property API endpoint:",
        apiEndpoint,
        "Property ID:",
        investment?.property?.id,
      );
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to update NOI calculations: ${errorData.error || "Unknown error"}\n\nDetails: ${errorData.details || "No additional details"}`,
        );
      }
    } catch (error) {
  ...
  const fetchInsuranceHistory = async () => {
    try {
      const response = await fetch(
        `/api/investors/insurance?propertyId=${investment?.property?.id}`,
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
  ...
  const fetchTaxHistory = async () => {
    try {
      const response = await fetch(
        `/api/investors/taxes?propertyId=${investment?.property?.id}`,
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
  ...
  const fetchWaterfallStructures = async () => {
    try {
      const response = await fetch(
        `/api/investors/waterfall-structures?propertyId=${investment?.property?.id}`,
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
  ...
  const fetchGlobalWaterfallStructures = async () => {
    try {
      const response = await fetch(
        "/api/investors/global-waterfall-structures",
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
  ...
  const fetchDistributions = async () => {
    ...
    try {
      ...
      const allResponse = await fetch(
        `/api/investors/waterfall-distributions/all?propertyId=${investment.property.id}`,
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
      ...
      if (allResponse.ok) {
        ...
        setWaterfallStructures((prev) =>
          prev.map((structure) => ({
            ...structure,
            waterfallDistributions: propertyDistributions.filter(
              (dist: any) => dist.waterfallStructureId === structure.id,
            ),
          })),
        );
      }
  ...
  const handleEditWaterfallStructure = (structure: any) => {
    console.log(
      "handleEditWaterfallStructure called with structure:",
      structure,
    );
  ...
  const handleUpdateWaterfallStructure = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/investors/waterfall-structures/${editingWaterfallStructure.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUpdateGlobalWaterfallStructure = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/investors/global-waterfall-structures/${editingWaterfallStructure.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteDistribution = async (distributionId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/investors/waterfall-distributions?id=${distributionId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
  ...
  const handleDeleteWaterfallStructure = async (structureId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this waterfall structure? This action cannot be undone.",
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/investors/waterfall-structures/${structureId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
  ...
  const handleEditDocument = (document: any) => {
    ...
    setEditDocumentData({
      ...
      visibleToAdmin:
        document.visibleToAdmin !== undefined ? document.visibleToAdmin : true,
      visibleToManager:
        document.visibleToManager !== undefined
          ? document.visibleToManager
          : true,
      visibleToInvestor:
        document.visibleToInvestor !== undefined
          ? document.visibleToInvestor
          : true,
    });
  ...
  const handleUpdateDocument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to update document: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
  ...
  const handleProcessDistribution = async (e: React.FormEvent) => {
    ...
    const isGlobalStructure = globalWaterfallStructures.some(
      (s) => s.id === distributionData.waterfallStructureId,
    );
    ...
    const requestBody = {
      ...
      ...(distributionData.distributionType === "REFINANCE" && {
        ...
        closingFeesItems: (distributionData.closingFeesItems || []).map(
          (i: any) => ({
            category: i.category,
    ...
    try {
      ...
      if (response.ok) {
        ...
        Object.entries(result.detailedBreakdown.byTier).forEach(
          ([tierName, tierData]: [string, any]) => {
            ...
            if (tierData.investors.length > 0) {
              ...
              tierData.investors.forEach((investor: any) => {
                ...
                const ownershipPercentage =
                  tierData.totalAmount > 0
                    ? (investor.amount / tierData.totalAmount) * 100
                    : 0;
        ...
        alert(
          `Failed to process distribution: ${errorData.error || "Unknown error"}\n\nDetails: ${errorData.details || "No additional details"}`,
        );
      }
    } catch (error) {
  ...
  const fetchDistributionBreakdown = async (distributionId: string) => {
    try {
      const response = await fetch(
        `/api/investors/waterfall-distributions/breakdown?id=${distributionId}`,
        {
          headers: {
            Authorization: `Bearer ${getAuthToken()}`,
          },
        ...
        <div className="text-center">
          ...
          <button
            onClick={() => {
              ...
              const savedTab =
                sessionStorage.getItem("investorDashboardActiveTab") ||
                "overview";
              router.push(`/investors/dashboard`);
            }}
                ...
                <button
                  onClick={() => {
                    ...
                    if (!investment) {
                      ...
                      alert(
                        "Investment data not loaded. Please refresh the page.",
                      );
                      return;
                    }
                    ...
                    try {
                      ...
                      const editDataToSet = {
                        ...
                        monthlyRent: (
                          investment.property.monthlyRent || 0
                        ).toString(),
                        capRate: (investment.property.capRate || 0).toString(),
                        occupancyRate: (
                          investment.property.occupancyRate || 0
                        ).toString(),
                        annualExpenses: (
                          investment.property.annualExpenses || 0
                        ).toString(),
                        currentValue: (
                          investment.property.currentValue || 0
                        ).toString(),
                        debtAmount:
                          investment.property.debtAmount?.toString() || "",
                        debtDetails: investment.property.debtDetails || "",
                        acquisitionPrice: (
                          investment.property.acquisitionPrice || 0
                        ).toString(),
                        constructionCost: (
                          investment.property.constructionCost || 0
                        ).toString(),
                        totalCost: (
                          investment.property.totalCost || 0
                        ).toString(),
                        acquisitionDate: investment.property.acquisitionDate
                          ? new Date(investment.property.acquisitionDate)
                              .toISOString()
                              .split("T")[0]
                          : "",
                        constructionCompletionDate: investment.property
                          .constructionCompletionDate
                          ? new Date(
                              investment.property.constructionCompletionDate,
                            )
                              .toISOString()
                              .split("T")[0]
                          : "",
                        stabilizationDate: investment.property.stabilizationDate
                          ? new Date(investment.property.stabilizationDate)
                              .toISOString()
                              .split("T")[0]
                          : "",
                        ...
                        bedrooms: (
                          investment.property.bedrooms || 0
                        ).toString(),
                        bathrooms: (
                          investment.property.bathrooms || 0
                        ).toString(),
                        squareFeet: (
                          investment.property.squareFeet || 0
                        ).toString(),
                        propertyType:
                          investment.property.propertyType || "RESIDENTIAL",
                        // Refinance fields - set to defaults for existing investments
                        isRefinance: false,
                        newLoanAmount: "",
                        oldLoanPayoff: "",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-6">
              ...
              {(() => {
                ...
                const totalFromOwners = propertyEntityInvestments.reduce(
                  (sum, ei) => {
                    // Use per-deal owners if available, otherwise fall back to global entity owners
                    const owners =
                      ei.entityInvestmentOwners &&
                      ei.entityInvestmentOwners.length > 0
                        ? ei.entityInvestmentOwners
                        : ei.entity?.entityOwners || [];
                    const ownersSum = owners.reduce(
                      (ownerSum: number, owner: any) =>
                        ownerSum + parseFloat(owner.investmentAmount || 0),
                      0,
                    );
                ...
                var displayInvestmentAmount =
                  totalFromOwners || investment.investmentAmount || 0;
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {investment.property.bedrooms} bed,{" "}
                            {investment.property.bathrooms} bath
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
                        <span
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(investment.status)}`}
                        >
                          {investment.status}
                        </span>
                      </div>
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {investment.property.debtAmount
                              ? formatCurrency(investment.property.debtAmount)
                              : "No debt recorded"}
                          </p>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
                  <div>
                    ...
                    <p className="font-medium">
                      {investment.property.squareFeet.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Monthly Rent</p>
                    <p className="font-medium">
                      {formatCurrency(investment.property.monthlyRent)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Cap Rate</p>
                    <p className="font-medium">
                      {formatPercentage(investment.property.capRate)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Occupancy</p>
                    <p className="font-medium">
                      {formatPercentage(investment.property.occupancyRate)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                <DealPhotoManager
                  ...
                  onThumbnailChange={(photoUrl) => {
                    ...
                    const thumbnailUpdates = JSON.parse(
                      sessionStorage.getItem("thumbnailUpdates") || "{}",
                    );
                    ...
                    sessionStorage.setItem(
                      "thumbnailUpdates",
                      JSON.stringify(thumbnailUpdates),
                    );
                  }}
                />
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    const totalInvestmentFromEntities =
                      propertyEntityInvestments.reduce(
                        (sum: number, ei: any) =>
                          sum + parseFloat(ei.investmentAmount || 0),
                        0,
                      );
                    ...
                    const constructionCost = parseFloat(
                      investment.property.constructionCost || 0,
                    );
                    ...
                    const equityToCostPercentage =
                      totalCost > 0
                        ? (totalInvestmentFromEntities / totalCost) * 100
                        : 0;
                    const currentValue = getCurrentNOICalculation();
                    const equityToValueAmount =
                      currentValue - (investment.property.debtAmount || 0);
                    const equityToValuePercentage =
                      currentValue > 0
                        ? (equityToValueAmount / currentValue) * 100
                        : 0;
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-blue-50 rounded-lg">
                          ...
                          <p className="text-sm text-blue-700 mt-1">
                            {formatPercentage(equityToCostPercentage)} of total
                            cost
                          </p>
                        </div>
                        <div className="p-4 bg-green-50 rounded-lg">
                          ...
                          <p className="text-sm text-green-700 mt-1">
                            {formatPercentage(equityToValuePercentage)} of
                            current value
                          </p>
                        </div>
                      </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">
                Investor Details & Entity Structure
              </h2>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-sm text-gray-600">
                        {propertyEntityInvestments.length} entit
                        {propertyEntityInvestments.length !== 1 ? "ies" : "y"}
                      </span>
                      <span className="text-sm text-blue-600 font-medium">
                        {propertyEntityInvestments.reduce(
                          (total: number, ei: any) =>
                            total + (ei.entity.entityOwners?.length || 0),
                          0,
                        )}{" "}
                  ...
                  {propertyEntityInvestments.map((entityInvestment) => (
                    <div
                      key={entityInvestment.id}
                      className="p-4 bg-gray-50 rounded-lg"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <h4 className="font-medium text-gray-900">
                            {entityInvestment.entity.name}
                          </h4>
                          <p className="text-sm text-gray-600">
                            {entityInvestment.entity.type}
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                          ...
                          <p className="font-medium text-gray-900">
                            {formatCurrency(entityInvestment.investmentAmount)}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-900">
                            {formatDate(entityInvestment.investmentDate)}
                          </p>
                        </div>
                      </div>
                      ...
                      {(() => {
                        ...
                        const hasPerDealOwners =
                          entityInvestment.entityInvestmentOwners &&
                          Array.isArray(
                            entityInvestment.entityInvestmentOwners,
                          ) &&
                          entityInvestment.entityInvestmentOwners.length > 0;
                        const hasGlobalOwners =
                          entityInvestment.entity?.entityOwners &&
                          Array.isArray(entityInvestment.entity.entityOwners) &&
                          entityInvestment.entity.entityOwners.length > 0;
                        const ownersToDisplay = hasPerDealOwners
                          ? entityInvestment.entityInvestmentOwners
                          : hasGlobalOwners
                            ? entityInvestment.entity.entityOwners
                            : [];
                          ...
                          <div className="mt-4">
                            <div className="flex items-center justify-between mb-3">
                              <h5 className="text-sm font-medium text-gray-700">
                                Individual Investors ({ownersToDisplay.length})
                              </h5>
                              <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
                                {ownersToDisplay.length} investor
                                {ownersToDisplay.length !== 1 ? "s" : ""}
                              </span>
                            </div>
                            <div className="space-y-3">
                              {ownersToDisplay.map((owner: any) => (
                                ...
                                >
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="bg-gray-50 p-3 rounded-lg">
                                      ...
                                      <p className="font-bold text-gray-900 text-lg">
                                        {formatCurrency(owner.investmentAmount)}
                                      </p>
                                    </div>
                                  ...
                                  {Array.isArray(owner.breakdown) &&
                                    owner.breakdown.length > 0 && (
                                      <div className="mt-3 pt-3 border-t border-gray-100">
                                        <button
                                          type="button"
                                          onClick={() =>
                                            setOpenOwnerBreakdownId(
                                        ...
                                        >
                                          {openOwnerBreakdownId === owner.id
                                            ? "Hide entity investment breakdown"
                                            : "View entity investment breakdown"}
                                        </button>
                                        {openOwnerBreakdownId === owner.id && (
                                  ...
                                  <div className="mt-3 pt-3 border-t border-gray-100">
                                    <div className="flex justify-between text-xs text-gray-500">
                                      <span className="font-medium">
                                        Entity: {entityInvestment.entity.name}
                                      </span>
                                      <span className="font-medium">
                                        Type: {entityInvestment.entity.type}
                                      </span>
                                    </div>
                                  </div>
                                </div>
                      ...
                      <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">
                                {investment.investorName || "Direct Investment"}
                              </p>
                              <p className="text-xs text-gray-600">
                                {investment.investorEmail || "N/A"}
                              </p>
                            </div>
                          </div>
                        ...
                        {propertyEntityInvestments.map((entityInvestment) => (
                          ...
                          >
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
                                {entityInvestment.entity.entityOwners?.length ||
                                  0}{" "}
                                investors
                              </span>
                            </div>
                            ...
                            {entityInvestment.entity.entityOwners &&
                              entityInvestment.entity.entityOwners.length >
                                ...
                                <div className="space-y-2">
                                  {entityInvestment.entity.entityOwners.map(
                                    (owner: any) => (
                                      <div
                                        key={owner.id}
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <p className="font-semibold text-gray-900">
                          {formatCurrency(
                            (investment.investmentType !== "ENTITY"
                              ...
                              : 0) +
                              propertyEntityInvestments.reduce(
                                (sum, ei) => sum + ei.investmentAmount,
                                0,
                              ),
                          )}
                        </p>
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="font-semibold text-gray-900">
                          {(investment.investmentType !== "ENTITY" ? 1 : 0) +
                            propertyEntityInvestments.reduce(
                              (sum, ei) =>
                                sum + (ei.entity.entityOwners?.length || 0),
                              0,
                            )}
                        </p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="font-semibold text-gray-900">
                          {propertyEntityInvestments.length}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {investment.property.acquisitionDate
                        ? formatDate(investment.property.acquisitionDate)
                        : "N/A"}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {formatCurrency(investment.property.acquisitionPrice)}
                    </p>
                  </div>
                  <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-6">
              ...
              {waterfallStructures.some(
                (structure) =>
                  structure.waterfallDistributions &&
                  structure.waterfallDistributions.length > 0,
              ) ? (
                <div className="space-y-4">
                  <h3 className="text-lg font-medium text-gray-800 mb-3">
                    Waterfall Distributions
                  </h3>
                  {waterfallStructures.map(
                    (structure) =>
                      structure.waterfallDistributions &&
                      structure.waterfallDistributions.length > 0 &&
                      structure.waterfallDistributions.map(
                        (distribution: any) => (
                          <div
                            key={distribution.id}
                              ...
                              <div>
                                <p className="font-medium text-gray-900">
                                  {distribution.description ||
                                    `Distribution from ${structure.name}`}
                                </p>
                                <p className="text-sm text-gray-600">
                                  {formatDate(distribution.distributionDate)}
                                </p>
                                <p className="text-xs text-blue-600">
                                  Waterfall: {structure.name}
                                </p>
                              </div>
                              <div className="text-right">
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  {(currentUser?.role === "ADMIN" ||
                                    ...
                                    <>
                                      <button
                                        onClick={() =>
                                          handleEditDistribution(distribution)
                                        }
                    ...
                    >
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-600">
                            {formatDate(distribution.distributionDate)} •{" "}
                            {distribution.waterfallStructure?.name ||
                              "Global Structure"}
                          </p>
                          {distribution.description && (
                            <p className="text-xs text-gray-500">
          ...
          <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
            ...
            {currentUser?.role === "ADMIN" && (
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-gray-600">
                      Active entities in this project:
                    </p>
                    <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                      {propertyEntityInvestments.length}{" "}
                      {propertyEntityInvestments.length === 1
                        ? "entity"
                        : "entities"}
                    </span>
                  </div>
                  ...
                  {propertyEntityInvestments.length > 0 ? (
                    <div className="space-y-3">
                      {propertyEntityInvestments.map((entityInvestment) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-xs text-blue-600">
                                ...
                                {formatPercentage(
                                  parseFloat(
                                    entityInvestment.ownershipPercentage || "0",
                                  ),
                                )}
                              </p>
                            </div>
                            <div className="flex items-center space-x-2">
                              ...
                              <button
                                onClick={() =>
                                  handleEditEntity(entityInvestment)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleDeleteEntity(entityInvestment.id)
                                }
                          ...
                          {/* Entity Owners - prefer per-deal owners if available */}
                          {((entityInvestment.entityInvestmentOwners &&
                            entityInvestment.entityInvestmentOwners.length >
                              0) ||
                            (entityInvestment.entity.entityOwners &&
                              entityInvestment.entity.entityOwners.length >
                                0)) && (
                    ...
                    <p className="text-sm text-gray-500 italic">
                      No entities active in this project
                    </p>
                  )}
            ...
            {/* NOI Calculator */}
            {((investment as any)?.property as any)?.["dealStatus"] !==
              "UNDER_CONSTRUCTION" && (
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-gray-900">
                  ...
                  {currentUser?.role !== "INVESTOR" && (
                    <button
                      onClick={() => {
                        setNoiData({
                          monthlyRent:
                            investment?.property?.monthlyRent?.toString() || "",
                          otherIncome:
                            investment?.property?.otherIncome?.toString() ||
                            "0",
                          annualExpenses:
                            investment?.property?.annualExpenses?.toString() ||
                            "",
                          capRate:
                            investment?.property?.capRate?.toString() || "",
                          currentValueEstimate:
                            investment?.property?.currentValue?.toString() ||
                            "",
                        });
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrency(
                        (investment?.property?.monthlyRent || 0) * 12,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrency(
                        (investment?.property?.otherIncome || 0) * 12,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatPercentage(investment?.property?.capRate || 0)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">
                      ...
                      {investment?.property?.capRate > 0
                        ? formatCurrency(
                            ((investment?.property?.monthlyRent || 0) * 12 +
                              (investment?.property?.otherIncome || 0) * 12 -
                              (investment?.property?.annualExpenses || 0)) /
                              (investment?.property?.capRate / 100),
                          )
                        : "N/A"}
                    </span>
                  </div>
                    ...
                    onClick={() => {
                      setNoiData({
                        monthlyRent:
                          investment?.property?.monthlyRent?.toString() || "",
                        otherIncome:
                          investment?.property?.otherIncome?.toString() || "0",
                        annualExpenses:
                          investment?.property?.annualExpenses?.toString() ||
                          "",
                        capRate:
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      value={
                        (investment as any)?.property?.dealStatus ||
                        "STABILIZED"
                      }
                      onChange={async (e) => {
                        ...
                        try {
                          await fetch(
                            `/api/investors/properties/${(investment as any)?.property?.id}`,
                            {
                              method: "PUT",
                              headers: {
                                "Content-Type": "application/json",
                  ...
                  <div>
                    ...
                    <select
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      value={
                        (investment as any)?.property?.fundingStatus || "FUNDED"
                      }
                      onChange={async (e) => {
                  ...
                  <div className="space-y-2">
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              Premium: {formatCurrency(insurance.annualPremium)}
                            </p>
                          </div>
                          <span className="text-xs text-gray-500">
                            {formatDate(insurance.renewalDate)}
                          </span>
                        </div>
                      </div>
                    ))}
                    {insuranceHistory.length > 3 && (
                      <p className="text-sm text-gray-500">
                        +{insuranceHistory.length - 3} more records
                      </p>
                    )}
                  </div>
                ) : (
              ...
              <div className="space-y-3">
                {taxHistory.length > 0 ? (
                  <div className="space-y-2">
                      ...
                      <div key={tax.id} className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              Amount: {formatCurrency(tax.annualPropertyTax)}
                            </p>
                          </div>
                          <span className="text-xs text-gray-500">
                    ...
                    {taxHistory.length > 3 && (
                      <p className="text-sm text-gray-500">
                        +{taxHistory.length - 3} more records
                      </p>
                    )}
                  </div>
                ) : (
              ...
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                ...
                <div className="space-y-3">
                  {waterfallStructures.length > 0 ? (
                    <div className="space-y-2">
                      {waterfallStructures
                        .slice(0, showAllWaterfallStructures ? undefined : 3)
                        .map((structure: any) => (
                          <div
                            key={structure.id}
                          ...
                          >
                            <div className="flex justify-between items-start">
                              <div>
                                  ...
                                  {(currentUser?.role === "ADMIN" ||
                                    ...
                                    <div className="flex space-x-2">
                                      ...
                                      <button
                                        onClick={() => {
                                          console.log(
                                            "Delete button clicked for structure:",
                                            structure.id,
                                            structure.name,
                                          );
                                          handleDeleteWaterfallStructure(
                                            structure.id,
                                          );
                                        }}
                                        className="p-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors duration-200"
                                ...
                                {structure.waterfallDistributions &&
                                  structure.waterfallDistributions.length >
                                    0 && (
                                    <div className="mt-2">
                                      <p className="text-xs font-medium text-gray-700 mb-1">
                                        Distributions:
                      ...
                      {waterfallStructures.length > 3 && (
                        <button
                          onClick={() => {
                            ...
                            console.log(
                              "Current showAllWaterfallStructures:",
                              showAllWaterfallStructures,
                            );
                            console.log(
                              "Total waterfall structures:",
                              waterfallStructures.length,
                            );
                            setShowAllWaterfallStructures(
                              !showAllWaterfallStructures,
                            );
                          }}
                      ...
                      <button
                        onClick={() => {
                          ...
                          const selectedStructureData =
                            waterfallStructures.find(
                              (s) =>
                                s.id === distributionData.waterfallStructureId,
                            ) ||
                            globalWaterfallStructures.find(
                              (s) =>
                                s.id === distributionData.waterfallStructureId,
                            );
                          if (
                            selectedStructureData &&
                            !selectedStructureData.propertyId
                          ) {
                            alert(
                              "Cannot process distributions from global structures. Please copy the structure to create a local one first.",
                            );
                            return;
                          }
            ...
            <form onSubmit={handleUploadDocument} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="ENVIRONMENTAL_REPORT">
                  ...
                  <div className="space-y-4">
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <p className="text-sm text-blue-800">
                        <strong>Note:</strong> Investment amount is calculated
                        from entity investments and cannot be edited here.
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditData({ ...editData, capRate: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        required
                      />
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const constructionCost =
                            parseFloat(editData.constructionCost) || 0;
                          const totalCost =
                            parseFloat(newAcquisitionPrice) + constructionCost;
                          setEditData({
                            ...editData,
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const acquisitionPrice =
                            parseFloat(editData.acquisitionPrice) || 0;
                          const totalCost =
                            acquisitionPrice + parseFloat(newConstructionCost);
                          setEditData({
                            ...editData,
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <p className="text-sm text-blue-800">
                        <strong>Note:</strong> Debt amount is managed in the
                        Property Loans section and cannot be edited here.
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
      ...
      {showUpdateEntityContactModal && pendingEntityContact && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-gray-900">
                Set Contact For: {pendingEntityContact.name}
              </h3>
              <span className="text-xs text-gray-500">
                Required before adding entity
              </span>
            </div>
            <form
              onSubmit={async (e) => {
                ...
                if (
                  !pendingEntityContact.contactPerson ||
                  pendingEntityContact.contactPerson.trim() === ""
                ) {
                  alert("Contact Person is required.");
                  return;
                }
                try {
                  const res = await fetch(
                    `/api/investors/entities/${pendingEntityContact.id}`,
                    {
                      method: "PUT",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  if (pendingEntitySelectionIndex !== null) {
                    const updatedEntity = availableEntities.find(
                      (e) => e.id === pendingEntityContact.id,
                    );
                    if (updatedEntity) {
                      updateEntityInvestor(
                        pendingEntitySelectionIndex,
                        "userId",
                        "",
                      );
                      updateEntityInvestor(
                      ...
                      updateEntityInvestor(
                        ...
                        (updatedEntity as any).entityOwners || [],
                      );
                      updateEntityInvestor(
                        pendingEntitySelectionIndex,
                        "user",
            ...
            <div className="space-y-4">
              <div>
                ...
                {(() => {
                  ...
                  const existingEntityIds = propertyEntityInvestments.map(
                    (ei: any) => String(ei.entityId),
                  );
                  const availableToAdd = availableEntities.filter(
                    (entity: any) =>
                      !existingEntityIds.includes(String(entity.id)),
                  );

                  if (availableToAdd.length === 0) {
                    return (
                      <p className="text-sm text-gray-500 italic py-4">
                        No other entities available. All existing entities are
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {availableToAdd.map((entity: any) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              handleAddExistingEntityToProperty(entity.id)
                            }
            ...
            <form onSubmit={handleCreateEntity} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntity({ ...newEntity, taxId: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntity({ ...newEntity, contactEmail: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntity({ ...newEntity, contactPhone: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
      ...
      {false && showMultiInvestorModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form
              onSubmit={handleCreateMultiInvestorInvestment}
              className="space-y-6"
            >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMultiInvestorData({
                        ...multiInvestorData,
                        entityOwnershipPercentage: e.target.value,
                      })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="0.0"
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${multiInvestorData.investors.reduce((sum, investor) => sum + parseFloat(investor.ownershipPercentage || "0"), 0) === 100 ? "text-green-600" : "text-red-600"}`}
                      >
                        {multiInvestorData.investors
                          .reduce(
                            (sum, investor) =>
                              sum +
                              parseFloat(investor.ownershipPercentage || "0"),
                            0,
                          )
                          .toFixed(1)}
                        %
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${multiInvestorData.investors.reduce((sum, investor) => sum + parseFloat(investor.investmentAmount || "0"), 0) <= parseFloat(multiInvestorData.totalInvestmentAmount || "0") ? "text-green-600" : "text-red-600"}`}
                      >
                        $
                        {multiInvestorData.investors
                          .reduce(
                            (sum, investor) =>
                              sum +
                              parseFloat(investor.investmentAmount || "0"),
                            0,
                          )
                          .toLocaleString()}
                      </span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${multiInvestorData.investors.some((investor) => parseFloat(investor.investmentAmount || "0") < 0) ? "text-red-600" : "text-green-600"}`}
                      >
                        {multiInvestorData.investors.some(
                          (investor) =>
      ...
      {showEditEntityModal && editingEntityInvestment && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-3xl mx-4 my-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Entity: {editingEntityInvestment.entity.name}
              </h3>
              <button
            ...
            <form onSubmit={handleUpdateEntityInvestment} className="space-y-4">
                      ...
                      onChange={(e) => {
                        ...
                        const updated = {
                          ...
                          entity: {
                            ...editingEntityInvestment.entity,
                            entityOwners:
                              editingEntityInvestment.entity.entityOwners.map(
                                (owner: any) => {
              ...
              <div className="bg-blue-50 rounded-lg p-4 mb-4">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    if (kind === "entity") {
                      ...
                      const alreadyAdded =
                        editingEntityInvestment.entity.entityOwners?.some(
                          (owner: any) =>
                      ...
                      if (
                        ...
                        String(selectedEntity.contactPerson).trim() === ""
                      ) {
                        setPendingEntitySelectionIndex(
                          editingEntityInvestment.entity.entityOwners?.length ||
                            0,
                        );
                      ...
                      try {
                        const entityResponse = await fetch(
                          `/api/investors/entities/${selectedEntity.id}`,
                          {
                            headers: {
                              Authorization: `Bearer ${getAuthToken()}`,
                            },
                        ...
                        const fullEntity = entityResponse.ok
                          ? await entityResponse.json()
                          : selectedEntity;

                        // Expand entity members into individual owners
                        const entityMembers =
                          fullEntity.entityOwners ||
                          (selectedEntity as any).entityOwners ||
                          [];
                        const newOwners = entityMembers.map((member: any) => ({
                          ...
                          investorEntityId: member.investorEntityId
                            ? String(member.investorEntityId)
                            : "",
                          isEntityInvestor: false,
                          entityName: "",
                          entityOwnersSnapshot: [],
                ...
                >
                  <option value="">
                    Select an entity or individual to add as investor...
                  </option>
                  ...
                  <optgroup label="Existing Entities">
                    {availableEntities
                      .filter(
                        (entity) =>
                          entity.id !== editingEntityInvestment.entity.id,
                      )
                      .map((entity) => (
                        <option
                          key={entity.id}
                          value={`entity:${String(entity.id)}`}
                        >
                          🏢 {entity.name}
                        </option>
                      ))}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Select an entity or individual to automatically add them as an
                  investor and view entity members below.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {editingEntityInvestment.entity.entityOwners &&
                  editingEntityInvestment.entity.entityOwners.length > 0 ? (
                    editingEntityInvestment.entity.entityOwners.map(
                          ...
                          <div className="space-y-2">
                            ...
                            {owner.investorEntityId ? (
                              <div className="text-sm text-gray-700 bg-gray-100 border border-gray-200 rounded-lg px-3 py-2">
                                ...
                                {owner.entityName ||
                                  availableEntities.find(
                                    (e) => e.id === owner.investorEntityId,
                                  )?.name ||
                                  "Selected entity"}
                                {Array.isArray(owner.entityOwnersSnapshot) &&
                                  owner.entityOwnersSnapshot.length > 0 && (
                                    <div className="mt-2 text-xs text-gray-600">
                                      <div className="font-semibold mb-1">
                                        Members preset in entity:
                                      </div>
                                      <ul className="list-disc ml-5 space-y-0.5">
                                        {owner.entityOwnersSnapshot.map(
                                          (m: any, i: number) => (
                                            <li key={i}>
                                              {m.user &&
                                              (m.user.firstName ||
                                ...
                                <div className="mt-3">
                                  ...
                                  >
                                    {owner.showBreakdown
                                      ? "Hide entity investment breakdown"
                                      : "Specify entity investment breakdown"}
                                  </button>
                                </div>
                                {owner.showBreakdown &&
                                  Array.isArray(owner.breakdown) && (
                                    <div className="mt-3 rounded border border-blue-200 bg-white p-3">
                                      ...
                                      <div className="mt-2 text-xs text-gray-700 flex items-center justify-between gap-3">
                                        <span>
                                          Breakdown total (auto-synced to
                                          investment amount)
                                        </span>
                                      ...
                                      <div className="text-[10px] text-gray-500 mt-1">
                                        This breakdown is specific to this deal
                                        and does not change entity membership.
                                      </div>
                                    </div>
                                  )}
                              </div>
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  title={
                                    ...
                                    Array.isArray(owner.breakdown) &&
                                    owner.breakdown.length > 0
                                      ? "Investment amount is auto-calculated from breakdown"
                                      : "Enter the investment amount for this investor"
                                  }
                                />
                              </div>
                              <div>
                                ...
                                <input
                                  ...
                                  title={
                                    owner.fromEntity === true
                                      ? "Ownership percentage comes from the entity and cannot be changed"
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        Click "Add Investor" to add the first investor.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-medium ${(editingEntityInvestment.entity.entityOwners || []).reduce((sum: number, owner: any) => sum + parseFloat(owner.ownershipPercentage || 0), 0) === 100 ? "text-green-600" : "text-red-600"}`}
                    >
                      {(editingEntityInvestment.entity.entityOwners || [])
                        .reduce(
                          (sum: number, owner: any) =>
                            sum + parseFloat(owner.ownershipPercentage || 0),
                          0,
                        )
                        .toFixed(1)}
                      %
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-medium ${(editingEntityInvestment.entity.entityOwners || []).reduce((sum: number, owner: any) => sum + parseFloat(owner.investmentAmount || 0), 0) <= parseFloat(editingEntityInvestment.investmentAmount || 0) ? "text-green-600" : "text-red-600"}`}
                    >
                      {formatCurrency(
                        (
                          editingEntityInvestment.entity.entityOwners || []
                          ...
                          (sum: number, owner: any) =>
                            sum + parseFloat(owner.investmentAmount || 0),
                          0,
                        ),
                      )}
                    </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNoiData({ ...noiData, monthlyRent: e.target.value })
                      }
                      readOnly={currentUser?.role === "INVESTOR"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNoiData({ ...noiData, otherIncome: e.target.value })
                      }
                      readOnly={currentUser?.role === "INVESTOR"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="0"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNoiData({ ...noiData, capRate: e.target.value })
                      }
                      readOnly={currentUser?.role === "INVESTOR"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      required
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-blue-900">
                        {formatCurrency(
                          parseFloat(noiData.monthlyRent || "0") * 12,
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-blue-900">
                        {formatCurrency(
                          parseFloat(noiData.otherIncome || "0") * 12,
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-blue-900">
                        {formatCurrency(
                          parseFloat(noiData.annualExpenses || "0"),
                        )}
                      </span>
                    </div>
                        ...
                        {parseFloat(noiData.capRate || "0") > 0
                          ? formatCurrency(
                              (parseFloat(noiData.monthlyRent || "0") * 12 +
                                parseFloat(noiData.otherIncome || "0") * 12 -
                                parseFloat(noiData.annualExpenses || "0")) /
                                (parseFloat(noiData.capRate || "0") / 100),
                            )
                          : "N/A"}
                      </span>
                    </div>
            ...
            <form onSubmit={handleAddTax} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTaxData({ ...taxData, taxYear: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              ...
              <div>
                ...
                <textarea
                  value={taxData.notes}
                  onChange={(e) =>
                    setTaxData({ ...taxData, notes: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  rows={3}
                />
      ...
      {showCreateWaterfallModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl mx-4 my-8">
            ...
            <form
              onSubmit={handleCreateWaterfallStructure}
              className="space-y-6"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Descriptive name for this tier (e.g., "8% Preferred
                            Return")
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            How this tier distributes funds (affects calculation
                            logic)
                          </p>
                          {tier.tierType === "PREFERRED_RETURN" && (
                            <p className="text-xs text-blue-600 mt-1 font-medium">
                              Preferred Return: Pays investors a fixed annual
                              return (e.g., 8%) before any other distributions
                            </p>
                          )}
                          {tier.tierType === "CATCH_UP" && (
                            <p className="text-xs text-blue-600 mt-1 font-medium">
                              Catch-Up: Ensures LPs receive their fair share
                              before GP promote kicks in
                            </p>
                          )}
                          {tier.tierType === "PROMOTE" && (
                            <p className="text-xs text-blue-600 mt-1 font-medium">
                              Promote: GP's share of profits after LPs receive
                              their preferred return and catch-up
                            </p>
                          )}
                          {tier.tierType === "RESIDUAL" && (
                            <p className="text-xs text-blue-600 mt-1 font-medium">
                              Residual: Remaining profits distributed based on
                              ownership percentages
                            </p>
                          )}
                          {tier.tierType === "RETURN_OF_CAPITAL" && (
                            <p className="text-xs text-blue-600 mt-1 font-medium">
                              Return of Capital: Returns original investment
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Annual return rate for preferred return tiers (e.g.,
                            8.0 = 8%)
                          </p>
                        </div>
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Percentage of profits to LP before GP promote (e.g.,
                            80.0 = 80%)
                          </p>
                        </div>
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Percentage of profits to GP after catch-up (e.g.,
                            20.0 = 20%)
                          </p>
                        </div>
                      </div>
              ...
              <form onSubmit={handleProcessDistribution} className="space-y-4">
                  ...
                  >
                    ...
                    <optgroup label="Local Structures">
                      {waterfallStructures.map((structure: any) => (
                        <option key={structure.id} value={structure.id}>
                          {structure.name} ({structure.waterfallTiers.length}{" "}
                          tiers)
                        </option>
                      ))}
                    </optgroup>
                    <optgroup label="Global Structures (Auto-Copy)">
                      {globalWaterfallStructures.map((structure: any) => (
                        <option key={structure.id} value={structure.id}>
                          {structure.name} ({structure.waterfallTiers.length}{" "}
                          tiers) - Global (will be copied automatically)
                        </option>
                      ))}
                    </optgroup>
                  </select>
                  <p className="text-xs text-gray-500 mt-1">
                    Choose the waterfall structure that defines how this
                    distribution will be split among investors
                  </p>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="INSURANCE_SETTLEMENT">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    The source of funds being distributed (affects waterfall
                    calculations)
                  </p>
                </div>
                  ...
                  <div className="space-y-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <div className="p-3 bg-blue-100 rounded-lg border border-blue-300 mb-4">
                      <p className="text-xs text-blue-800 font-medium">
                        💡 <strong>How it works:</strong> Enter the refinance
                        amount and fees. The system will:
                      </p>
                      <ul className="text-xs text-blue-700 mt-2 ml-4 list-disc">
                        <li>Set the new debt amount to the refinance amount</li>
                        <li>
                          Calculate distribution as: Refinance Amount - Previous
                          Debt - All Fees
                        </li>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const originationFees =
                            parseFloat(distributionData.originationFees) || 0;
                          const closingFees =
                            parseFloat(distributionData.closingFees) || 0;
                          const prepaymentPenalty =
                            parseFloat(distributionData.prepaymentPenalty) || 0;

                          // Get current debt amount from the investment's property
                          const currentDebtAmount =
                            investment?.property?.debtAmount || 0;
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Total refinance amount (this becomes the new debt
                        amount)
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const refinanceAmount =
                            parseFloat(distributionData.refinanceAmount) || 0;
                          const originationFees =
                            parseFloat(e.target.value) || 0;
                          const closingFees =
                            parseFloat(distributionData.closingFees) || 0;
                      ...
                      <div className="space-y-2">
                        {distributionData.closingFeesItems.map((item, idx) => (
                          ...
                          >
                            <input
                              ...
                              onChange={(e) => {
                                const items = [
                                  ...distributionData.closingFeesItems,
                                ];
                                items[idx] = {
                                  ...items[idx],
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const refinanceAmount =
                                  parseFloat(
                                    distributionData.refinanceAmount,
                                  ) || 0;
                                const originationFees =
                                  parseFloat(
                                    distributionData.originationFees,
                                  ) || 0;
                                const prepaymentPenalty =
                                  parseFloat(
                                    distributionData.prepaymentPenalty,
                                  ) || 0;
                                const closingFeesSum = items.reduce(
                                  (s, it) =>
                                    s + (parseFloat(it.amount || "0") || 0),
                                  0,
                                );
                                const currentDebtAmount =
                                  investment?.property?.debtAmount || 0;
                                ...
                                setDistributionData({
                                  ...
                                  totalAmount: distributionAmount.toString(),
                                });
                              }}
                            />
                            <button
                              ...
                              onClick={() => {
                                const items =
                                  distributionData.closingFeesItems.filter(
                                    (_, i) => i !== idx,
                                  );
                                const refinanceAmount =
                                  parseFloat(
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-sm text-gray-700">
                            Total: $
                            {parseFloat(
                              distributionData.closingFees || "0",
                            ).toLocaleString("en-US", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                        ...
                        <p className="text-xs text-gray-500">
                          Add one or more closing fee line items with
                          categories.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                      ...
                      <p className="text-xs text-green-600 mt-1">
                        Refinance Amount - Previous Debt - Origination Fees -
                        Closing Fees - Prepayment Penalty
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4 my-8">
            ...
            <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm text-blue-800">
                <strong>Edit Distribution:</strong> Update the details of this
                distribution. Note: Changing the amount will recalculate the
            ...
            <form onSubmit={handleUpdateDistribution} className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  The total amount to be distributed (e.g., $50,000 in rental
                  income)
                </p>
              </div>
              <div>
      ...
      {showCreateGlobalWaterfallModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl mx-4 my-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-900">
                Create Global Waterfall Structure
              </h3>
              <button
            ...
            <div className="mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
              <p className="text-sm text-purple-800">
                <strong>Global Structure:</strong> This structure can be applied
                to any entity investment. It will be copied and customized for
                each entity it's applied to.
              </p>
            </div>
            <form
              onSubmit={handleCreateGlobalWaterfallStructure}
              className="space-y-6"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            {tier.tierType === "PREFERRED_RETURN" &&
                              "Guaranteed return to investors before profits are split"}
                            {tier.tierType === "CATCH_UP" &&
                              "Allows GP to catch up to their promote percentage"}
                            {tier.tierType === "PROMOTE" &&
                              "GP share of profits after preferred return and catch-up"}
                            {tier.tierType === "RESIDUAL" &&
                              "Remaining profits split based on ownership percentages"}
                            {tier.tierType === "RETURN_OF_CAPITAL" &&
                              "Return of original invested capital to investors"}
                          </p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
            ...
            <div className="mb-4 p-4 bg-orange-50 rounded-lg border border-orange-200">
              <p className="text-sm text-orange-800">
                <strong>Apply Structure:</strong> Select a global waterfall
                structure to apply to an entity investment. This will create a
                copy of the structure specific to that entity.
              </p>
            </div>
            <form
              onSubmit={handleApplyWaterfallStructure}
              className="space-y-4"
            >
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setApplyWaterfallData({
                      ...applyWaterfallData,
                      entityInvestmentId: e.target.value,
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
      ...
      {showEditWaterfallModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl mx-4 my-8">
            ...
            <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm text-blue-800">
                <strong>Edit Structure:</strong> Modify the waterfall structure
                for this property. Changes will affect all future distributions
                using this structure.
              </p>
            </div>
            <form
              onSubmit={handleUpdateWaterfallStructure}
              className="space-y-6"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                        ...
                        {tier.tierType === "CATCH_UP" && (
                          <div>
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">
                              Percentage of profits GP receives during catch-up
                              (e.g., 80% = 80.0)
                            </p>
                          </div>
                        )}
                        ...
                        {tier.tierType === "PROMOTE" && (
                          <div>
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">
                              GP's promote percentage of profits (e.g., 20% =
                              20.0)
                            </p>
                          </div>
                        )}
              ...
              <div className="flex flex-col space-y-3 pt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    console.log(
                      "Delete button clicked, editingWaterfallStructure:",
                      editingWaterfallStructure,
                    );
                    if (
                      !editingWaterfallStructure ||
                      !editingWaterfallStructure.id
                    ) {
                      alert(
                        "Error: No waterfall structure selected for deletion",
                      );
                      return;
                    }
                    if (
                      ...
                      console.log(
                        "Deleting waterfall structure with ID:",
                        editingWaterfallStructure.id,
                      );
                      handleDeleteWaterfallStructure(
                        editingWaterfallStructure.id,
      ...
      {showEditGlobalWaterfallModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl mx-4 my-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-900">
                Edit Global Waterfall Structure
              </h3>
              <button
            ...
            <div className="mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
              <p className="text-sm text-purple-800">
                <strong>Edit Global Structure:</strong> Modify the global
                waterfall structure. Changes will affect all entity investments
                that use this structure.
              </p>
            </div>
            <form
              onSubmit={handleUpdateGlobalWaterfallStructure}
              className="space-y-6"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
              ...
              <div className="flex space-x-3 pt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        "Are you sure you want to delete this global waterfall structure? This action cannot be undone.",
                      )
                    ) {
                      handleDeleteWaterfallStructure(
                        editingWaterfallStructure.id,
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setEditDocumentData({
                          ...editDocumentData,
                          visibleToAdmin: e.target.checked,
                        })
                      }
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setEditDocumentData({
                          ...editDocumentData,
                          visibleToManager: e.target.checked,
                        })
                      }
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setEditDocumentData({
                          ...editDocumentData,
                          visibleToInvestor: e.target.checked,
                        })
                      }
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
            ...
            <div className="space-y-6">
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-blue-900">
                        {breakdownData.detailedBreakdown.summary.tiersProcessed}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-lg font-bold text-blue-900">
                        {breakdownData.detailedBreakdown.summary.totalInvestors}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-700">Entities</p>
                      <p className="text-lg font-bold text-blue-900">
                        {breakdownData.detailedBreakdown.summary.totalEntities}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {breakdownData.detailedBreakdown?.byTier && (
                <div className="space-y-4">
                  ...
                  {Object.entries(breakdownData.detailedBreakdown.byTier).map(
                    ([tierName, tierData]: [string, any]) => (
                      <div
                        key={tierName}
                        className="border border-gray-200 rounded-lg p-4"
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-lg font-bold text-green-600">
                              {formatCurrency(tierData.totalAmount)}
                            </p>
                          </div>
                        </div>
                        ...
                        {/* Individual Investors */}
                        {tierData.investors &&
                          tierData.investors.length > 0 && (
                            <div className="mb-3">
                              <h6 className="text-sm font-medium text-gray-700 mb-2">
                                👤 Individual Investors
                              </h6>
                              <div className="space-y-2">
                                {tierData.investors.map(
