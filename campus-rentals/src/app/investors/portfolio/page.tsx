  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8" style={{ fontFamily: 'var(--font-sans)' }}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-semibold text-ink-900 tracking-tight">Portfolio</h1>
          <p className="text-[15px] text-ink-500 mt-1">Holdings and performance</p>
        </div>
        <a
          href={ABODINGO_WEBSITE_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-ink-200 bg-white text-ink-700 text-[14px] font-medium hover:bg-ink-50 hover:border-ink-300 transition-colors shadow-sm"
        >
      ...
      <div className="bg-white rounded-2xl shadow-soft ring-1 ring-ink-900/5 overflow-hidden">
        <div className="px-6 py-4 border-b border-ink-100 flex items-center gap-2">
          <BuildingOffice2Icon className="w-5 h-5 text-ink-500" />
          <h2 className="text-[22px] font-semibold text-ink-900 tracking-tight">Holdings</h2>
        </div>
        <div className="overflow-x-auto">
          {investments.length === 0 ? (
            <div className="px-6 py-12 text-center text-ink-500 text-[15px]">
              ...
              <tbody>
                {investments.map((inv) => (
                  <tr key={inv.id} className="border-b border-ink-100 hover:bg-ink-50">
                    ...
                    <td className="px-6 py-4 text-ink-600 text-[15px]">{inv.status}</td>
                    ...
                    <td className="px-6 py-4 text-right text-ink-600 text-[15px]">
