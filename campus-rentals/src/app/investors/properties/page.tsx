...
export default function InvestorsPropertiesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6" style={{ fontFamily: 'var(--font-sans)' }}>
      <div>
        <h1 className="text-2xl sm:text-3xl font-semibold text-ink-900 tracking-tight">Properties</h1>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 border border-ink-200 rounded-xl focus:ring-2 focus:ring-accent/20 focus:border-accent text-ink-900"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-soft ring-1 ring-ink-900/5 p-10 text-center">
          <BuildingOffice2Icon className="h-12 w-12 text-ink-300 mx-auto mb-3" />
        ...
        <ul className="space-y-3">
          {filtered.map((p) => (
            <li key={p.id}>
              <div className="bg-white rounded-2xl shadow-soft ring-1 ring-ink-900/5 p-4 sm:p-5 flex flex-wrap items-center justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <h2 className="text-lg font-semibold tracking-tight text-ink-900 truncate">{p.name}</h2>
                  {p.address && (
                    <div className="flex items-center gap-1.5 mt-1 text-ink-600 text-sm">
