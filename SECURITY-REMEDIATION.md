...
- [x] **Money columns `Float` → `Decimal`** — DONE (schema + code sweep), Strategy A (behavior-preserving). 45 money columns → `Decimal(15,2)` in `schema.prisma` + migration `011_money_to_decimal.sql`; all server-side money arithmetic wrapped in `Number(...)` so compute behavior is **identical to today** but DB storage is now exact. tsc has **0 Decimal-arithmetic errors**; full build green. **STILL REQUIRED before merge/apply:** (1) apply `011` ALTER against the DB, (2) verify a real waterfall distribution end-to-end in staging, (3) note `fund_contributions.amount` / `fund_distributions.amount` were left as `Float` (separable follow-up), and full Decimal-*compute* exactness (vs behavior-preserving) is a future refinement.
- [ ] ~~Money columns `Float` → `Decimal`~~ (superseded by the line above) — original caution preserved: In Prisma, `Decimal` returns a `Prisma.Decimal` object at runtime (not a JS `number`), so every `+`/`*`/`reduce` on these fields (heavy in the waterfall math) breaks silently — and `ignoreBuildErrors: true` means the compiler won't catch it. This needs: (1) a code sweep converting money arithmetic to Decimal ops or `.toNumber()`, (2) a tested `ALTER TABLE ... TYPE numeric(15,2)` migration with correct precision (money `(15,2)`, rates/percentages a different scale). Best done together with the Supabase migration. Do NOT flip the schema alone.
